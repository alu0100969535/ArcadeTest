...
void AInvaderSquad::UpdateSquadState(float delta) {
	...
	for (auto invader : SquadMembers) {
		...
		if (invader) { // very important, first nullptr is checked!.
			...
			UInvaderMovementComponent* imc = static_cast<UInvaderMovementComponent*>(invader->GetComponentByClass(UInvaderMovementComponent::StaticClass()));
	...
	if (countSurvivors>0 && val < (1.0 - FMath::Exp(-freeJumpRate * this->timeFromLastFreeJump))) {
		...
		UInvaderMovementComponent* imc = static_cast<UInvaderMovementComponent*>(survivors[ind]->GetComponentByClass(UInvaderMovementComponent::StaticClass()));
		if (imc) {
...
void AInvaderSquad::SquadFinishesDown() {
	...
	if (countActions >= numberOfMembers) {
		...
		case InvaderMovementType::RIGHT:
			...
			GEngine->AddOnScreenDebugMessage(-1, 1, FColor::Blue, FString::Printf(TEXT("Changing direction LEFT")));
			break;
		case InvaderMovementType::LEFT:
			state = InvaderMovementType::RIGHT;
			GEngine->AddOnScreenDebugMessage(-1, 1, FColor::Blue, FString::Printf(TEXT("Changing direction RIGHT")));

			break;
		default:
			state = InvaderMovementType::STOP;
			GEngine->AddOnScreenDebugMessage(-1, 1, FColor::Blue, FString::Printf(TEXT("Changing direction STOP")));
